...
def load_grayscale_image(
    image_path, size=None, normalize=False, background_color="white"
):
    """
...
def compare_grayscale_reconstruction(
    ...
    *,
    ...
    if original_img.ndim != 2:
        raise ValueError(
            f"Expected a 2D grayscale original image, got shape {original_img.shape}."
        )
    ...
    if verbose:
        ...
        print(
            f"Retrieved range: [{reconstructed_img.min()}, {reconstructed_img.max()}]"
        )
        print("\n Error analysis:")
